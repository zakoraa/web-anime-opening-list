<div class="absolute w-full top-0 left-0 flex justify-between items-center h-20 px-5 lg:px-20">
        <img class="rounded-full border-2 border-white" src="{{ asset('storage/Z.jpg') }}" alt="hallo" width="70px">
        <div class="flex w-52 lg:w-56 justify-between">
            <a href="{{ route('signup') }}" class="rounded-lg cursor-pointer hover:bg-[#0000009c] transition duration-300 py-2 px-4">Sign Up</a>
            <a href="{{ route('login') }}" class="py-2 px-4 rounded-lg cursor-pointer hover:bg-[#0000009c] transition duration-300">Login </a>
        </div>
    </div>